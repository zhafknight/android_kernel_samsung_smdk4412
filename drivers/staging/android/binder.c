...
static int binder_state_show(struct seq_file *m, void *unused)
{
	...
	hlist_for_each_entry(device, pos, &binder_devices, hlist) {
		...
		hlist_for_each_entry(node, pos1, &context->binder_dead_nodes,
				     dead_node)
			print_binder_node(m, node);
	...
	hlist_for_each_entry(device, pos, &binder_devices, hlist) {
		...
		hlist_for_each_entry(proc, pos1, &context->binder_procs, proc_node)
...
static int binder_proc_show(struct seq_file *m, void *unused)
{
	...
	hlist_for_each_entry(device, pos, &binder_devices, hlist) {
		...
		hlist_for_each_entry(itr, pos1, &context->binder_procs, proc_node) {
			if (itr->pid == pid) {
