	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	rc = kstrtoul(buf, (unsigned int)0,
				(unsigned long *)&value);
	if (rc < 0)
		return rc;
	else {
...
#endif
	lcd_fb_suspend = s6e63m0_fb_suspend;
	lcd_fb_resume = s6e63m0_fb_resume;
...
static int s6e63m0_remove(struct device *dev)
{
	struct lcd_info *lcd = dev_get_drvdata(dev);
